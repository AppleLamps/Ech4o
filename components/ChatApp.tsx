...
const exampleTitle = "Ech4o";
const visibleModelName = "openai/gpt-5.4-nano";
const STORAGE_KEY = "ech4o-conversations-v1";
const SIDEBAR_STORAGE_KEY = "ech4o-sidebar-v1";
...
function clientRequestBudget(responseMode: RequestMode) {
  const outputTokens =
    responseMode === "extended"
      ? EXTENDED_MAX_OUTPUT_TOKENS
      : STANDARD_MAX_OUTPUT_TOKENS;
  return (
    (MODEL_CONTEXT_TOKENS - outputTokens - RESERVED_SYSTEM_AND_OVERHEAD_TOKENS) *
    APPROX_CHARS_PER_TOKEN
  );
}

function trimMessagesForRequest(messages: Message[], responseMode: RequestMode) {
  const budget = clientRequestBudget(responseMode);
